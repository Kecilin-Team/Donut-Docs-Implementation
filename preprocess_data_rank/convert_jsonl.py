...
json_file = "RankData.json"
with open(json_file, 'r') as f:
    data = json.load(f)    

# file JSONL
output_file = "RankData.jsonl"  
    ...
    for item in data:
        ...
        try:
            for i in range(len(item['transcription'])):
                ...
                if label_name[0] == "Award Name":
                    if "Award_Name" not in gt_parse: 
                        gt_parse["Award_Name"] = []
                    gt_parse["Award_Name"].append(item['transcription'][i])
                    
                if label_name[0] == "Award Date":
                    if "Award_Date" not in gt_parse: 
                        gt_parse["Award_Date"] = []
                    gt_parse["Award_Date"].append(item['transcription'][i])
